...
class FontUtil {
  ...
  /// [_initialFontModel]本地存储键，如果用户在[init]中更改了[FontModel]，则重新加载新的字体
  ...
  /// 当前加载的全局字体模型
  static FontModel _currentFontModel = FontModel.systemFont;
  ...
  /// 初始化全局默认字体
  static Future<List<String>?> initFont([FontModel? fontModel]) async {
    ...
    if (localStr != null) {
      ...
      if (initialLocalStr != null) {
        _initialFontModel = FontModel.fromJson((jsonDecode(initialLocalStr) as Map).cast<String, dynamic>());
        ...
        _initialFontModel = fontModel ?? FontModel.systemFont;
        localStorage.setItem(_initialLocalKey, jsonEncode(_initialFontModel.toJson()));
      }
      if (allowLoadLocalFont) {
        late FontModel localFontModel;
        try {
          localFontModel = FontModel.fromJson((jsonDecode(localStr) as Map).cast<String, dynamic>());
